...
economy_keywords <- c("economy", "economic", "econs")
art_keywords <- c("art", "creative")
science_keywords <- c("science", "scientific", "chemistry", "biology", "biomedical", "biotechnology ")
business_keywords <- c("business")

# Grupiranje po kategorijama (Grouping by categories)
participants$category <- participants$field

# Check for matches in 'participants$field' and assign corresponding categories
if (any(agrep(paste(economy_keywords, collapse = "|"), participants$field, ignore.case = TRUE) > 0)) {
  participants$category <- "Economy"
} else if (any(agrep(paste(art_keywords, collapse = "|"), participants$field, ignore.case = TRUE) > 0)) {
  participants$category <- "Art"
} else if (any(agrep(paste(science_keywords, collapse = "|"), participants$field, ignore.case = TRUE) > 0)) {
  participants$category <- "Science"
} else if (any(agrep(paste(business_keywords, collapse = "|"), participants$field, ignore.case = TRUE) > 0)) {
  participants$category <- "Business"
}
# If no match, retain the original category
